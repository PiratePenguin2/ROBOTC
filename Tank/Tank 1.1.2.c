...
const float armLower = -10;
const float armHold  = 20;

const float clawOpen  = 25;
//const float clawClose = -30;
const float clawGrip  = -70;
...
void tankDrive()
{
    ...
    //Right Motor
    {
        if (abs(vexRT[Ch2]) >= tol){
            motor[rightMotor] = 0.87 * vexRT[Ch2] * tankMaxSpeed / 127;
        } else {
            motor[rightMotor] = 0;
        }
    }   //Right Motor
...
void joystickDrive()
{
  ...
  targetSpeedL = (drive + steer);  // (y + x)/2
	targetSpeedR = (drive - steer) * 0.87;  // (y - x)/2
