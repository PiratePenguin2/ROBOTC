...
/*-----Config-----*/
	const float rearMultiplier = 0; //This multiplier is used for the back wheel, to ensure that it is pivoting at a similar speed to the outside wheels.
...
task main()
{
	...
	/*Take inputs from the joystick, and determine how the bot should move*/
		...
		//Forwards
			else if ((vexRT[Ch4] >= 0) && (vexRT[Ch3] >= jitter))					//Turn Clockwise, Right motor small multiplier
			{
				targetSpeedL = vexRT[Ch3];
				targetSpeedR = (vexRT[Ch3] * ((-2 * vexRT[Ch4]) + motorMax)/motorMax);
  		}
  		else if ((vexRT[Ch4] <= 0) && (vexRT[Ch3] >= jitter))					//Turn Counter-Clockwise, Left motor small multiplier
  		{
